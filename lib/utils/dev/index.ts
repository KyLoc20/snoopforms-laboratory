...
export { fetchWithAgent };
//TODO
const PROXY = (process.env.NODE_ENV !== "production" ? process.env.PROXY_ENDPOINT : "") ?? "http://127.0.0.1:7890"; //"http(s)://{host}:{port}"
type FetchOptions = {
  method: "POST";
  body?: any;
};
const fetchWithAgent = (url: string, options: FetchOptions) => {
  const { method, body } = options;
  console.log("fetchWithAgent", PROXY);
  return fetch(url, {
    method,
...
function DiscordWebhookExample() {
  fetchWithAgent(`https://discord.com/api/webhooks/1024027447332720640/6ZzLIAaCZM4v-SL-OX6hicNubpPN_xmqG-zEyUY0BY087KVlCgq-D5QHA0su-xJIT_Mz`, {
    method: "POST",
