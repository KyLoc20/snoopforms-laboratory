...
export default class TextQuestion implements BlockTool {
  ...
  _toggleTune(tune: any) {
    ...
    if (tune === "required") {
      ...
      const nodeToUpdate = this.wrapper?.childNodes[0]?.childNodes[0]?.childNodes[1];
      if (nodeToUpdate) nodeToUpdate.textContent = this.data.required ? "*" : "";
    }
  }

  render(): HTMLElement {
    this.wrapper = document.createElement("div");
    const styleStr = `.hello{
        background: red;
    }`;
    const toolView = (
      <div className="pb-5">
        <div className="relative font-bold leading-7 text-gray-800 text-md sm:truncate">
          <input
            ...
            className="hello w-full p-0 border-0 border-transparent ring-0 focus:ring-0 placeholder:text-gray-300"
            placeholder="Your Question"
          />

          <div className="absolute inset-y-0 right-0 flex items-center pr-3 text-red-500 pointer-events-none">*</div>
        </div>
        <input
          type="text"
